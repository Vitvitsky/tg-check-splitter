...
async def finish_voting(callback: CallbackQuery, state: FSMContext, db: AsyncSession):
    ...
    if unvoted:
        ...
        for item in unvoted:
            lines.append(f"- {item.name} -- {item.price} rub")
...
async def settle_session(callback: CallbackQuery, state: FSMContext, db: AsyncSession, bot: Bot):
    ...
    shares = calculate_shares(items_data, session.tip_percent)

    for member in session.members:
        share = int(shares.get(member.user_tg_id, 0))
        await bot.send_message(
            member.user_tg_id,
            f"Сессия завершена!\nТвоя доля: {share} rub (включая {session.tip_percent}% чаевых)",
        )
...
async def _format_results(db: AsyncSession, session_id: str, tip_percent: int) -> str:
    ...
    for user_id, amount in sorted(shares.items(), key=lambda x: -x[1]):
        ...
        lines.append(f"{name} -- {int(amount)} rub")

    raw_total = sum(i.price for i in session.items)
    tip_amount = total - raw_total if tip_percent else 0

    lines.append("-" * 20)
    lines.append(f"Всего: {int(total)} rub")
    if tip_percent:
        lines.append(f"(из них чаевые: {int(tip_amount)} rub)")
