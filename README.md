...
| `api`     | 127.0.0.1:8005 | REST, WebSocket, Mini App |
| `bot`     | —    | Telegram-бот, polling |
| `db`      | 127.0.0.1:5433 | PostgreSQL 17 |

Оба порта публикуются **только на loopback**: наружу смотрит host nginx, он проксирует
на `127.0.0.1:8005`. Порт базы опубликован не для приложения (внутри сети она доступна
как `db:5432`), а для тестов конкурентности — `TEST_DATABASE_URL` ходит с хоста.
