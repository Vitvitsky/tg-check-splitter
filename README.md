...
## API: Управление данными

### Сброс счётчика сканирований

Обнуляет использованные бесплатные сканирования текущего пользователя (счётчик `free_scans_used`). Платные сканирования не затрагиваются.

```bash
curl -X POST /api/quota/reset \
  -H "Authorization: tma <initData>"
```

**Ответ:** `{"ok": true}`

В Mini App этот endpoint доступен через хук `useResetQuota()`.

### Очистка истории сессий

Удаляет все завершённые (`settled`) сессии, где текущий пользователь — администратор. Активные сессии не затрагиваются.

```bash
curl -X DELETE /api/sessions/history \
  -H "Authorization: tma <initData>"
```

**Ответ:** `{"deleted": <количество удалённых>}`

В Mini App кнопка «Clear All» отображается в секции History на главной странице.
